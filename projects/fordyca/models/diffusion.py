...
def crwD_for_searching(N: float, wander_speed: float, scenario: str) -> float:
    """
...
def crwD_for_avoiding(N: float, wander_speed: float, scenario: str) -> float:
    """
    Approximates the diffusion constant in a swarm of N CRW robots for bounded arena geometry for
    collision avoidance. From :xref:`Harwell2021a`, inspired by the results in :xref:`Codling2010`.

    """
    D = crwD_for_searching(N, wander_speed, scenario) * 1.0 / 0.055
