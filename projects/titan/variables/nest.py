...
class Nest():
    ...
    """
    Defines the position/size/etc of the nest based on block distribution type. Exactly ONE nest is
        ...
        if not self.attr_changes:
            if self.dist_type == 'SS':
                self.attr_changes = [XMLAttrChangeSet(
                    ...
                    XMLAttrChange(".//arena_map/nests/nest",
                                  "light_height",
                                  "{0:.9f}".format(math.pow(s.area(), 0.25))),
                    XMLAttrChange(".//arena_map/nests/nest",
                                  "light_intensity",
                                  "{0:.9f}".format(math.pow(s.area(), 0.5))),
